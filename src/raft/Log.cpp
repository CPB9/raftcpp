...
bmcl::Option<Error> LogCommitter::entry_append(const Entry& ety, bool needVoteChecks)
{
    ...
    bool is_voting_cfg_change = ety.entry.isFirst() && ety.entry.unwrapFirst().is_voting_cfg_change();
    if (needVoteChecks && is_voting_cfg_change && voting_change_is_in_progress())
