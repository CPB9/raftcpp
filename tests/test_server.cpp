...
{
    raft::Server r(raft::NodeId(1), true, &__Sender, &__Saver);
    r.nodes().add_node(raft::NodeId(0));
    r.nodes().add_node(raft::NodeId(2));
    r.set_current_term(1);
    ...
    /* vote for ID 0 */
    r.vote_for_nodeid(raft::NodeId(0));
    {
        MsgVoteRep rvr = r.accept_req(raft::NodeId(2), MsgVoteReq(1, 1, 1));
        EXPECT_EQ(ReqVoteState::NotGranted, rvr.vote_granted);
    }
...
TEST(TestCandidate, receives_majority_of_votes_becomes_leader)
{
    ...
    r.accept_rep(raft::NodeId(3), MsgVoteRep(1, ReqVoteState::Granted));
    EXPECT_TRUE(r.is_leader());
}
...
TEST(TestLeader, recv_appendentries_response_increase_commit_idx_using_voting_nodes_majority)
{
    ...
    r.nodes().add_node(raft::NodeId(2));
    r.nodes().add_node(raft::NodeId(3));
    r.nodes().add_non_voting_node(raft::NodeId(4));
...
TEST(TestLeader, recv_appendentries_response_jumps_to_lower_next_idx)
{
    ...
    /* receive mock success responses */
    r.accept_rep(raft::NodeId(2), MsgAppendEntriesRep(2, false, 1, 0));
...
TEST(TestLeader, recv_appendentries_response_decrements_to_lower_next_idx)
{
    ...
    /* receive mock success responses */
    r.accept_rep(raft::NodeId(2), MsgAppendEntriesRep(2, false, 4, 0));
...
TEST(TestLeader, recv_requestvote_responds_without_granting)
{
    ...
    EXPECT_EQ(ReqVoteState::NotGranted, opt.vote_granted);
}
