...
TEST(TestServer, election_timeout_does_promote_us_to_leader_if_there_is_only_1_node)
{
    ...
    /* clock over (ie. 1000 + 1), causing new election */
    r.raft_periodic(r.timer().get_election_timeout() + std::chrono::milliseconds(1));
...
TEST(TestFollower, recv_appendentries_increases_log)
{
    ...
    /* include one entry */
    MsgAddEntryReq ety(2, 1, raft::EntryData("aaa", 4));
...
TEST(TestFollower, recv_appendentries_reply_false_if_doesnt_have_log_at_prev_log_idx_which_matches_prev_log_term)
{
    ...
    /* include one entry */
    MsgAddEntryReq ety(r.get_current_term()-1, 1, raft::EntryData("aaa", 4));
    ae.entries = &ety;
    ae.n_entries = 1;
...
TEST(TestFollower, recv_appendentries_delete_entries_if_conflict_with_new_entries_via_prev_log_idx)
{
    ...
    MsgAddEntryReq mety = MsgAddEntryReq(r.get_current_term() - 1, 4, EntryData(str4));
    ae.entries = &mety;
    ae.n_entries = 1;
...
TEST(TestFollower, recv_appendentries_delete_entries_if_conflict_with_new_entries_via_prev_log_idx_at_idx_0)
{
    ...
    /* include one entry */
    std::vector<uint8_t> str4 = { 4, 4, 4 };
    MsgAddEntryReq mety = Entry(0, 4, EntryData(str4));
    ae.entries = &mety;
    ae.n_entries = 1;
...
TEST(TestFollower, recv_appendentries_add_new_entries_not_already_in_log)
{
    ...
    /* include entries */
    MsgAddEntryReq e[2] = {Entry(0, 1), Entry(0, 2) };
    ae.entries = e;
    ae.n_entries = 2;
...
TEST(TestFollower, recv_appendentries_set_commitidx_to_prevLogIdx)
{
    ...
    /* include entries */
    MsgAddEntryReq e[4] = {Entry(1, 1), Entry(1, 2), Entry(1, 3), Entry(1, 4)};
    ae.entries = e;
    ae.n_entries = 4;
...
TEST(TestFollower, recv_appendentries_does_not_deleted_commited_entries)
{
    ...
    MsgAppendEntriesReq ae(1, 0, 1, 0);
    /* include entries */
    MsgAddEntryReq e[7] = {Entry(1, 1), Entry(1, 2), Entry(1, 3), Entry(1, 4), Entry(1, 5), Entry(1, 6), Entry(1, 7) };
    ae.entries = &e[0];
    ae.n_entries = 1;
    {
        auto aer = r.accept_req(raft::NodeId(2), ae);
...
TEST(TestLeader, when_it_becomes_a_leader_sends_empty_appendentries)
{
    ...
    r.raft_periodic(r.timer().get_request_timeout());
    msg = sender.poll_msg_data(r);
    EXPECT_TRUE(msg.isSome());
    if (msg.isSome())
    {
...
TEST(TestLeader, sends_appendentries_with_prevLogIdx)
{
    ...
    /* add 1 entry */
    /* receive appendentries messages */
    r.log().entry_append(MsgAddEntryReq(2, 100, raft::EntryData("aaa", 4)));
    n->set_next_idx(1);
    r.send_appendentries(n->get_id());
    {
        bmcl::Option<msg_t> msg = sender.poll_msg_data(r);
...
TEST(TestLeader, sends_appendentries_when_node_has_next_idx_of_0)
{
    ...
    /* receive appendentries messages */
    n->set_next_idx(1);
    r.log().entry_append(MsgAddEntryReq(1, 100, raft::EntryData("aaa", 4)));
    r.send_appendentries(n->get_id());
    {
        bmcl::Option<msg_t> msg = sender.poll_msg_data(r);
        EXPECT_TRUE(msg.isSome());
...
TEST(TestLeader, recv_appendentries_response_duplicate_does_not_decrement_match_idx)
{
    ...
    /* append entries - we need two */
    r.log().entry_append(MsgAddEntryReq(1, 1, raft::EntryData("aaa", 4)));
    r.log().entry_append(MsgAddEntryReq(1, 2, raft::EntryData("aaa", 4)));
    r.log().entry_append(MsgAddEntryReq(1, 3, raft::EntryData("aaa", 4)));
...
TEST(TestLeader, recv_appendentries_response_jumps_to_lower_next_idx)
{
    ...
    /* append entries */
    r.log().entry_append(MsgAddEntryReq(1, 1, raft::EntryData("aaa", 4)));
    r.log().entry_append(MsgAddEntryReq(2, 2, raft::EntryData("aaa", 4)));
    r.log().entry_append(MsgAddEntryReq(3, 3, raft::EntryData("aaa", 4)));
    r.log().entry_append(MsgAddEntryReq(4, 4, raft::EntryData("aaa", 4)));
...
TEST(TestLeader, recv_appendentries_response_retry_only_if_leader)
{
    ...
    /* append entries - we need two */
    raft::TermId term = r.get_current_term();
    r.log().entry_append(MsgAddEntryReq(term, 1, raft::EntryData("aaa", 4)));
...
TEST(TestLeader, recv_entry_is_committed_returns_neg_1_if_invalidated)
{
    ...
    MsgAddEntryReq e(2, 999, raft::EntryData("aaa", 4));
    ae.entries = &e;
    ae.n_entries = 1;
    auto aer = r.accept_req(raft::NodeId(2), ae);
    EXPECT_TRUE(aer.isOk());
...
TEST(TestLeader, recv_requestvote_responds_without_granting)
{
    ...
    MsgVoteRep rvr = r.accept_req(raft::NodeId(3), MsgVoteReq(1, 0, 0));
    EXPECT_EQ(raft::ReqVoteState::NotGranted, rvr.vote_granted);
}
